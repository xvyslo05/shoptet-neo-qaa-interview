...
| TC-NAMEDAY-045 | AC10.c / AC9 | Scenario | P1 | component | auto | Fill date *and* name, then submit | No client-side block — both values reach `requestNameday`; the mocked `AMBIGUOUS_QUERY` error is shown (resolved by the implementation: `NamedayForm.component.test.tsx`, "submits with both date and name present") |
...
| TC-NAMEDAY-052 | AC1, AC2 | Scenario | P1 | e2e | auto | Open app, submit valid date → result; reset; submit valid name → result | Both results render end-to-end through the real UI + real backend (`e2e/cypress/e2e/nameday_lookup.e2e.cy.ts`) |
| TC-NAMEDAY-053 | AC5, AC7.a, AC11.a | Scenario | P2 | e2e | auto | Submit invalid date → error shown; click Reset → error cleared (`e2e/cypress/e2e/nameday_lookup.e2e.cy.ts`); submit today's date | Error shown, then cleared by Reset, then replaced by the correct result |
...
| AC11 | 046, 053 | — | 047 (concurrency) | component, e2e |
...
- **Closed during code review (unit/api test review):** a repeated `date` or `name` query param arrives at Express as an array, not a string — verified live against the running server (`?date=1.1.&date=2.2.` → 400 `INVALID_DATE`; `?name=Tomas&name=Petr` → 404 `NAME_NOT_FOUND`). The `typeof === "string"` guard in `app.ts` already handles this safely, but nothing pinned it down, so a future refactor could drop the guard unnoticed (the *Delinquent* review lens: malformed input, not just invalid values). New atomic conditions AC5.e/AC7.b, closed by TC-NAMEDAY-058/059. Also closed: the contract test (`nameday-contract.api.test.ts`) validated only the 400 `BadRequestError` schema against `openapi.yaml`; AC14.a covers both 400 and 404, but the `NameNotFoundError` schema was never contract-tested — closed by TC-NAMEDAY-060. Both `nameday.api.test.ts` and `nameday-contract.api.test.ts` also had identical `app.listen(0)`/close boilerplate, since extracted into a shared `backend/tests/testServer.ts` helper.
- **Closed during code review (component test review):** `frontend/src/api.ts` — the module AC14.a/b are actually about (the `isApiFailure` type guard, the fallback-message template) — had **no test of its own** anywhere in the repo. Every `NamedayForm.component.test.tsx` case mocks `requestNameday` wholesale, so those tests prove the form reacts correctly to whatever error it's handed, not that `api.ts` derives the right message from a real HTTP response. Concretely, TC-NAMEDAY-049's mocked message string was hand-copied from `api.ts`'s own fallback template — if that template changed, the component test would keep passing against its own stale copy. Closed by TC-NAMEDAY-061/062/063 in `frontend/tests/unit/apiClient.unit.test.ts`, exercising `requestNameday` against a real `msw/node` server instead of a hand-mocked one. Also fixed: TC-NAMEDAY-045's expected result used to read "test documents whichever the implementation actually does" — the component test now shows definitively that both fields reach the API unblocked; wording updated to state that as fact. Also fixed: `apiClient.unit.test.ts` shipped 5 cases but only 3 (061–063) had a TC ID — the other two (request-building for a date query, URL-encoding a diacritic name) were untraceable. Added as TC-NAMEDAY-064/065.
- **Closed during e2e review:** AC11.a says Reset clears "fields/result/**error**", but every existing Reset test — TC-NAMEDAY-046 (component) and the original draft of TC-NAMEDAY-053 — only ever clicked Reset after a *successful* result, never after an actual populated error message. Nothing in the whole suite proved Reset clears an error. TC-NAMEDAY-053 now clicks Reset between the invalid-date error and the corrected resubmission (instead of clearing the date field in place) and asserts the error is empty right after — closing that gap and giving AC11 its first e2e coverage.
- **Closed during e2e review:** AC11.a says Reset clears "fields/result/**error**", but every existing Reset test — TC-NAMEDAY-046 (component) and the original draft of TC-NAMEDAY-053 — only ever clicked Reset after a *successful* result, never after an actual populated error message. Nothing in the whole suite proved Reset clears an error. TC-NAMEDAY-053 now clicks Reset between the invalid-date error and the corrected resubmission (instead of clearing the date field in place) and asserts the error is empty right after — closing that gap and giving AC11 its first e2e coverage.
...
- **API** (`backend/tests/api/nameday.api.test.ts`, `contracts/tests/api/nameday-contract.api.test.ts`): the 6 decision-table rules, the code-order check, the malformed-shape cases, and the two contract-schema checks (TC-NAMEDAY-033–040, 058–060) — proves the HTTP contract independent of the UI.
